...
<a href="<?= BASE_URL . "Potrjena" ?>">Zgodovina potrjenih naročil</a>
...
<div id="main">
    ...
    $sql = "SELECT A.NAZIV_ARTIKLA, N.STATUS_NAROCILA, N.KOLICINA_ARTIKLA, N.ZAPOREDNA_STEVILKA, N.ID_NAROCILA  FROM NAROCILO N, VSEBUJE V, ARTIKEL A
        WHERE N.ID_NAROCILA = V.ID_NAROCILA AND A.SIFRA_ARTIKLA = V.SIFRA_ARTIKLA AND N.STATUS_NAROCILA = 2 ORDER BY N.ZAPOREDNA_STEVILKA";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            <li><a href="<?= BASE_URL . "Posamezno?idnar=" . $row["ID_NAROCILA"] ?>">Naziv: <?= $row["NAZIV_ARTIKLA"] ?>, Status: 
